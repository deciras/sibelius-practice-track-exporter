# Sibelius 练习音轨导出器 / Sibelius Practice Track Exporter
...
从 Sibelius 乐谱批量导出排练/练习音轨。这个插件主要面向阿卡贝拉资料准备流程：每个 staff 导出一条练习音轨，可选打击乐/VP 编组，并可额外导出 FullMix。

### 插件版本

- `ExportDimPracticeTracks.plg`：图形界面版，推荐大多数用户使用。
- `ExportDimPracticeTracksNoGUI.plg`：稳定无界面版，通过修改插件代码顶部变量来配置，适合固定流程的一键导出。
...
Batch-export rehearsal and practice tracks from a Sibelius score. This plugin is designed for a cappella preparation workflows where you need one track per staff, optional percussion/VP grouping, and a full mix.

### Plugin Versions

- `ExportDimPracticeTracks.plg`: GUI version. Recommended for most users.
- `ExportDimPracticeTracksNoGUI.plg`: stable no-GUI version. Settings are edited at the top of the plugin code. Useful for a fixed one-click workflow.
...
3. Fully quit and reopen Sibelius.
...
```text
/Users/<your-user-name>/Library/Application Support/Avid/Sibelius/Plugins/批处理
```
